...
const HistoryList: React.FC<HistoryListProps> = ({ links, isLoading, onViewStats }) => {
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {links.map((link) => {
              ...
              return (
                <tr key={link.short_code} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <span className="text-indigo-600 font-medium mr-2">{fullShortLink}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="max-w-xs truncate text-gray-500 text-sm" title={link.url}>
                      {link.url}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
